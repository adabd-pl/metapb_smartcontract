...
class GraphSmartContract extends Contract {
    ...
    **/
    ...
    async deletePermissions(ctx, sourceVertexId, destinationVertexId) {
        ...
        destinationVertexJson.direct_children_index_table = destinationVertexJson.direct_children_index_table.filter(id => id !== sourceVertexId);
    ...
    async updatePermissions(ctx, sourceVertexId, destinationVertexId, permissions, operation) {
        ...
        try {
            ...
            console.log(destinationVertexJson)
            await this.updatePermissionsHelper(ctx, sourceVertexJson, destinationVertexJson, permissions, operation, sourceChildrenIds, destinationParentIds);
 ...
 */
    async updatePermissionsHelper(ctx, sourceVertexJson, destinationVertexJson, permissions, operation, sourceChildrenIds, destinationParentIds) {
        let vertexExists = false;

        try {
            vertexExists = this.updatePermissionsInParentIndexTable(sourceVertexJson.effective_parent_index_table, destinationVertexJson.vertex) || vertexExists;
            vertexExists = this.updatePermissionsInParentIndexTable(sourceVertexJson.direct_parent_index_table, destinationVertexJson.vertex) || vertexExists;
            vertexExists = this.updatePermissionsInIndexTable(destinationVertexJson.effective_children_index_table, sourceVertexJson.vertex, permissions) || vertexExists;
            vertexExists = this.updatePermissionsInIndexTable(destinationVertexJson.direct_children_index_table, sourceVertexJson.vertex, permissions) || vertexExists;
        } catch (err) {
        ...
        if (!vertexExists) {
            console.log('Vertex does not exist in index tables, adding new vertex...');
        ...
        try {
            console.log(destinationVertexJson, destinationParentIds)
            await this.topDownPropagation(ctx, sourceVertexJson, "TOP_DOWN", operation,destinationParentIds,sourceVertexJson );
            await this.topDownPropagation(ctx, destinationVertexJson, "BOTTOM_UP", operation,sourceChildrenIds  , destinationVertexJson);
        } catch (err) {
    ...
    */
    ...
    async topDownPropagation(ctx, vertex, direction, operation, effectiveNeighbors, intermediary) {
        console.log(`Starting propagation for vertex: ${vertex.vertex}, direction: ${direction}, operation: ${operation}`);
        ...
        effectiveNeighbors.forEach(e => {
            ...
            if (direction === "BOTTOM_UP" && recalculatePrivileges) {
                console.log(`Calculating permissions for ${gamma.vertex}, intermediaries: ${gamma.intermediaries}`);
            ...
            console.error(`Błąd dla sąsiada ${e}:`, error);
        }
        ...
        if (toPropagate.length > 0) {
            ...
            for (const s of successors) {
                ...
                console.log("compositeKeySuccessor: ", compositeKeySuccessor);
        ...
        try{
            ...
            console.error(`Błąd podczas zapisywania wierzchołka ${vertex.vertex}:`, error);
        }
    ...
    async deleteAllFromState(ctx) {
        ...
        for await (const { key, value } of ctx.stub.getStateByPartialCompositeKey(objectType, [])) {
            ...
            // If we’ve reached the end of the iterator, break the loop
            if (value) {
